    ...
    for i in range(layers, 0, -1):
        if i==layers:
            if loss=="bce":
                ...
                gradients['da_'+str(i)] = -np.sum(y/activations['a_'+str(i)], axis=1, keepdims=True)
                gradients['dz_'+str(i)] = np.multiply(gradients['da_'+str(i)], softmax_derivative(activations['z_'+str(i)]))
            elif loss=="mse":
                gradients['da_'+str(i)] = np.mean(y-activations['a_'+str(i)]-y)
        ...
        gradients['dw_'+str(i)] = (1/m)*np.dot(gradients['dz_'+str(i)], activations['a_'+str(i-1)].T)
